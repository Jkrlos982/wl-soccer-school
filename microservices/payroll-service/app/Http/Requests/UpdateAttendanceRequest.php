...
class UpdateAttendanceRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Validate attendance record exists and can be modified
            $attendanceId = $this->route('attendance');
            if ($attendanceId) {
                $attendance = \App\Models\Attendance::find($attendanceId);
                if ($attendance) {
                    // Check if attendance is from more than 30 days ago
                    $attendanceDate = Carbon::parse($attendance->date);
                    if ($attendanceDate->diffInDays(now()) > 30) {
                        $validator->errors()->add('date', 'No se puede modificar asistencia de más de 30 días de antigüedad.');
                    }
                    
                    // Check if payroll has been processed for this period
                    $payrollExists = \App\Models\Payroll::where('employee_id', $attendance->employee_id)
                        ->whereHas('payrollPeriod', function ($query) use ($attendanceDate) {
                            $query->where('start_date', '<=', $attendanceDate)
                                  ->where('end_date', '>=', $attendanceDate)
                                  ->where('status', 'closed');
                        })->exists();
                    
                    if ($payrollExists) {
                        $validator->errors()->add('date', 'No se puede modificar asistencia de un período de nómina ya procesado.');
                    }
                }
            }
        });

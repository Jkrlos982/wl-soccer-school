...
class UpdateDepartmentRequest extends FormRequest
{
            ...
            'parent_id' => [
                ...
                function ($attribute, $value, $fail) use ($departmentId) {
                    if ($value == $departmentId) {
                        $fail('Un departamento no puede ser padre de sí mismo.');
                    }
                }
            ],
            'budget' => 'sometimes|numeric|min:0|max:999999999.99',
            'cost_center' => [
                'sometimes',
                'nullable',
                'string',
                'max:50',
                Rule::unique('departments', 'cost_center')->ignore($departmentId)
            ],
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->has('parent_id') && $this->input('parent_id')) {
                ...
                if ($this->wouldCreateCircularDependency($departmentId, $this->input('parent_id'))) {
                    $validator->errors()->add('parent_id', 'Esta asignación crearía una dependencia circular.');
                }
            }
            ...
            if ($this->has('status') && $this->input('status') === 'inactive' && $department) {
                $activeEmployees = $department->employees()->where('status', 'active')->count();
                if ($activeEmployees > 0) {
                    $validator->errors()->add('status', 'No se puede desactivar un departamento con empleados activos.');
                }
            }
        });
    }
    ...
    /**
     * Check if assigning a parent would create a circular dependency.
     */
    private function wouldCreateCircularDependency($departmentId, $parentId, $visited = [])
    {
        ...
        $visited[] = $parentId;
        $parent = \App\Models\Department::find($parentId);
        ...
        return $this->wouldCreateCircularDependency($departmentId, $parent->parent_id, $visited);
    }
